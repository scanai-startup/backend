CREATE TABLE IF NOT EXISTS `enchimento`(
  ...
  `ausensiapeiradespaletizacao` TINYINT NOT NULL,
  ...
  `ausensiaaguaenxaguadora` TINYINT NOT NULL,
  ...
  CONSTRAINT `fk_tb_engarrafamento_1`
    FOREIGN KEY (`fkvinho`)
    REFERENCES `mydb`.`tb_vinho` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_tb_enchimento_1`
    FOREIGN KEY (`fkrespproducao`)
    REFERENCES `mydb`.`tb_funcionario` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
